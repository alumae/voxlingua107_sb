...
class SpeakerBrain(sb.core.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        loss = self.hparams.compute_cost(predictions, langid, lens)

        if hasattr(self.hparams.lr_annealing, "on_batch_end"):
            self.hparams.lr_annealing.on_batch_end(self.optimizer)

        if stage != sb.Stage.TRAIN:
            self.error_metrics.append(uttid, predictions, langid, lens)
...
def dataio_prep_shards(hparams):
    ...
    label_encoder.load_or_create(
        path=lab_enc_file, from_iterables=[train_meta["language_ids"]], output_key="lang_id",
    )
    ...
    def audio_pipeline(sample_dict: Dict, random_chunk=True):
        ...
        spk_id_idx = label_encoder.encode_sequence_torch([meta["language_id"]])

        return {
            "sig": sig,
            "lang_id_encoded": spk_id_idx,
            "id": key,
        }
