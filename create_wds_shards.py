...
def write_shards(
    ...
    for f in audio_files:
        # path should be
        # $voxceleb_folder_path/<LANG_ID>/<VIDEO---0000.000-0000.000.wav> 
        #breakpoint()
        m = re.match("(.*/((.+)/.+---(\d\d\d\d\.\d\d\d)-(\d\d\d\d\.\d\d\d))\.wav)", f.as_posix())
        if m:
          loc = m.group(1)
          key = m.group(2)
          lang = m.group(3)
    ...
    language_id_to_idx = {
        lang: idx
        for idx, lang in enumerate(all_language_ids)
    }
    ...
    ## swap the language id for the language_id index in each tuple
    #data_tuples = [
    #    (key, language_id_to_idx[language_id], f, dur)
    #    for key, language_id, f, dur in data_tuples
    #]
